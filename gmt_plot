...
function config_psxy_timeseries(){
    ...
    echo "psxy_G=black" >> ${config}
    echo "## 設定回歸線樣式" >> ${config}
    echo "Plot_Mean_PS_W=true" >> ${config}
    echo "psxy_W=3p,red,8_8:2p" >> ${config}
...
function plot_timeseries(){
    ...
    gmt gmtset TIME_SYSTEM MJD
    DateMJDArray=(`cat ${Input_Date} | awk '{printf("%d\n",$1)}' | gmt gmtconvert -fiT -fot`)
    ...
    do
        ...
        echo ${DateMJDArray[${j}]} ${MeanArray[${j}]} >> tmp_TS_Mean_MJD.txt
    done
    gmt regress tmp_TS_Mean_MJD.txt -Fxmc -C95 > tmp_regress.txt
    if [ "${Plot_Mean_PS_W}" == "true" ];then
        Edge_Left_MJD=`echo ${Edge_Left} | gmt gmtconvert -fiT -fot`
        Edge_Right_MJD=`echo ${Edge_Right} | gmt gmtconvert -fiT -fot`
        gmt psxy -R${Edge_Left_MJD}/${Edge_Right_MJD}/${Edge_Lower}/${Edge_Upper} -J ${psxy_W} -O -K tmp_regress.txt >> ${Output_File}
    fi
    gmt psxy -R${Edge_Left}/${Edge_Right}/${Edge_Lower}/${Edge_Upper} -J -S${psxy_Size_Mean} -G${psxy_G} ${errorbar} -O -K tmp_TS_Mean_Error.txt >> ${Output_File}
